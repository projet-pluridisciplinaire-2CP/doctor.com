...
export class HypotheseDiagnosticRepository {
  ...
  async getRecentSuivisByPatient(
    ...
    return database
      ...
      .from(suivi)
      .where(
        and(eq(suivi.patient_id, patientId), ne(suivi.id, currentSuiviId)),
      )
...
export const hypotheseDiagnosticRepository = new HypotheseDiagnosticRepository();
